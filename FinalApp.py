...
class SharedState:
    ...
    def __init__(self):
        ...
        self.latest_audio = None                     # data-URI wav string from alarm
    ...
    def add_alarm(self, payload):
        with self.lock:
            ...
            # Resolve this alarm's snapshot (data-URI / base64 / url)
            snap_b64 = payload.get("snapshot_b64")
            snap_saved = camera.get("saved_snapshot")
            this_snapshot = None
            if snap_b64:
                this_snapshot = ("b64", snap_b64)
            elif isinstance(snap_saved, str) and snap_saved.startswith("data:image"):
                this_snapshot = ("b64", snap_saved)
            elif isinstance(snap_saved, str) and snap_saved.startswith("http"):
                this_snapshot = ("url", snap_saved)

            # Resolve this alarm's audio clip
            audio_b64 = audio.get("audio_b64") or payload.get("audio_b64")
            this_audio = audio_b64 if (isinstance(audio_b64, str)
                                       and audio_b64.startswith("data:audio")) else None
            ...
            self.alarms.append({
                ...
                "snapshot": this_snapshot,   # media attached to THIS alarm
                "audio": this_audio,
            })
            # Keep "latest" pointers for the top panel
            if this_snapshot:
                self.latest_snapshot = this_snapshot
            if this_audio:
                self.latest_audio = this_audio
            self.last_msg_time = time.time()
...
with snap_col:
    ...
    # Intruder audio recording, if the Pi embedded it in the alarm
    if audio_clip:
        st.caption("🔊 Captured audio at last alarm")
        try:
            st.audio(audio_clip)
        except Exception:  # noqa: BLE001
            st.caption("Audio received but could not be played.")
...
if alarms:
    # Overview table (quick scan of all events)
    ...
    # Per-alarm media: click a time to see that exact snapshot + audio clip
    st.caption("Open an event to view the photo and hear the audio captured at that "
               "exact trigger time:")
    for a in reversed(alarms):
        label = f"🕑 {a['timestamp']}  ·  {a['pathways'] or '—'}  ·  cam {a['cam_confidence']:.0f}%"
        with st.expander(label):
            mcol1, mcol2 = st.columns([1, 1])
            with mcol1:
                snap = a.get("snapshot")
                if snap and snap[0] == "b64":
                    try:
                        st.image(snap[1], use_container_width=True)
                    except Exception:  # noqa: BLE001
                        st.caption("Image could not be displayed.")
                elif snap and snap[0] == "url":
                    st.image(snap[1], use_container_width=True)
                else:
                    st.caption("No image for this alarm.")
            with mcol2:
                if a.get("audio"):
                    st.caption("🔊 Audio at this trigger")
                    try:
                        st.audio(a["audio"])
                    except Exception:  # noqa: BLE001
                        st.caption("Audio could not be played.")
                else:
                    st.caption("No audio for this alarm.")
                st.markdown(f"**Audio z-score:** {a['z_score']:.2f} σ")
                st.markdown(f"**Top class:** {a['best_class_1']}")
                st.markdown(f"**AoI breached:** {'YES' if a['aoi_breached'] else 'no'}")
else:
